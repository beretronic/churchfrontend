...
const TitheForm = () => {
    ...
    // ===>>> CORRECTED THIS LINE <<<===
    const titheApiEndpoint = 'http://localhost:8000/api/finances/tithes/'; // Correct endpoint based on your DRF setup
    ...
    const handleSubmit = async (e) => {
        ...
        if (!tithe.tither || !tithe.amount || !tithe.month) {
            setMessage('Please fill in all required fields.');
            setIsError(true);
            return;
        }

        const parsedAmount = parseFloat(tithe.amount);
        ...
        const dataToSend = { ...tithe, amount: parsedAmount };
        ...
        try {
            const response = await fetch(titheApiEndpoint, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                setMessage('Tithe submitted successfully!');
                setIsError(false);
                setTithe({ tither: '', amount: '', month: '' }); // Clear form
                ...
                console.error('Tithe submission failed:', response.status, errorData);

                let errorMessage = 'Failed to submit tithe. An unknown error occurred.';
                if (errorData) {
                    if (errorData.detail) {
                        errorMessage = `Failed to submit tithe: ${errorData.detail}`;
                    } else {
                        const fieldErrors = Object.keys(errorData)
                            .map(key => `${key}: ${Array.isArray(errorData[key]) ? errorData[key].join(', ') : errorData[key]}`)
                            .join('\n');
                        errorMessage = `Failed to submit tithe: ${fieldErrors}`;
                    }
                }
            ...
            console.error('Network error during tithe submission:', error);
    ...
    return (
        <form onSubmit={handleSubmit} style={styles.form}>
            ...
            <h2 style={styles.heading}>Add Tithe</h2>
            <input
                style={styles.input}
                name="tither"
                placeholder="Tither Name"
                value={tithe.tither}
            ...
            >
                {isSubmitting ? 'Submitting Tithe...' : 'Submit Tithe'}
            </button>
        </form>
    );
};
